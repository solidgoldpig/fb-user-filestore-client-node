...
class FBUserFileStoreClient extends FBJWTClient {
  ...
  async fetch (args, logger) {
    ...
    const json = await this.sendGet({url, context, payload}, logger)
    const {file} = json
  ...
  async store (args, logger) {
    ...
    const result = await this.sendPost({url, context, payload}, logger)
...
FBUserFileStoreClient.offline = () => {
  class FBUserFileStoreClientOffline extends FBUserFileStoreClient {
    async store (args) {
  ...
  return new FBUserFileStoreClientOffline('SERVICE_SECRET', 'SERVICE_TOKEN', 'SERVICE_SLUG', 'SUBMITTER_URL')
}
